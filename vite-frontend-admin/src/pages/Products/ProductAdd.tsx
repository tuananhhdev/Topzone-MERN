...
interface IColor {
  color: string;
  price?: number; // Chỉ có với phone
  stock?: number; // Chỉ có với phone
  variantImage?: string[];
  fileList?: UploadFile[];
}

interface IVariant {
  storage?: string; // Chỉ có với phone
  product_name?: string;
  colors: IColor[];
}
...
const ProductAdd: React.FC = () => {
  ...
  const handleRemoveColor = useCallback(
    (storageIndex: number, colorIndex: number) => {
      setVariants((prev) => {
        const newVariants = [...prev];
        newVariants[storageIndex].colors = newVariants[
          storageIndex
  ...
  const handleColorChange = useCallback(
    (storageIndex: number, colorIndex: number, field: string, value: string | number) => {
      setVariants((prev) =>
        prev.map((variant, index) =>
          index === storageIndex
            ? {
  ...
  const onFinish = async (values: any) => {
    try {
          ...
          const updatedColors = await Promise.all(
            variant.colors.map(async (color) => {
              ...
              return {
                ...
                variantImage: uploadedColorImages,
              };
            })
          );
      ...
      const specification = {
        ...
        camera: values.specification.camera || {
          main: '',
          selfie: '',
          features: [],
        },
        graphics_card: values.specification.graphics_card || {
          model: '',
          vram: '',
        },
...
const setInputRef = (index: number) => (el: HTMLInputElement | null) => {
  ...
  const VariantForm = () => {
    if (productType === 'phone') {
      // Variant cho phone: có dung lượng (storage)
      return (
        <>
          <Collapse
            accordion={false}
              ...
              <Collapse.Panel
                ...
                extra={
                  <Button
                    ...
                    danger
                    onClick={(e) => {
                      e.stopPropagation(); // Ngăn sự kiện click lan lên Collapse
                    ...
                    <Col span={12}>
                      <Form.Item label="Dung lượng" required>
                        <Input
                          ...
                          onChange={(e) => {
                            handleStorageChange(storageIndex, 'storage', e.target.value);
      ...
      // Variant cho laptop: chỉ có màu sắc và hình ảnh
      return (
        <>
          <Collapse
            accordion={false}
      ...
      >
        ...
        >
          <Select
            ...
            onChange={(value) => {
              ...
              setVariants([]); // Reset variants khi thay đổi product type
            }}
          >
        ...
        <Form.Item>
          <Button
            type="primary"
            htmlType="submit"
            loading={createMutationProduct.isPending}
          >
            {createMutationProduct.isPending ? 'Adding...' : 'Add Product'}
          </Button>
        </Form.Item>
      </Form>
    </>
